...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'Laravel') }} - Esqueci minha senha</title>
    ...
    <body class="landing-bg min-h-screen text-white antialiased">
        <div class="relative z-10 max-w-4xl mx-auto px-6 py-12">
            ...
            <main class="grid gap-8 lg:grid-cols-2 items-start">
                <div class="space-y-6">
                    <h1 class="text-3xl font-extrabold">Esqueceu sua senha?</h1>
                    <p class="text-white/75">Informe seu e-mail e enviaremos um código de verificação para redefinir sua senha.</p>
                </div>

                <div class="hero-panel rounded-2xl p-6">
                    @php
                        $resetEmail = old('email');
                        $resetCooldown = $resetEmail
                            ? \Illuminate\Support\Facades\RateLimiter::availableIn('password-reset-email:'.sha1(\Illuminate\Support\Str::lower((string) $resetEmail)))
                            : (int) session('password_reset_available_in', 0);
                    @endphp

                    <div class="mb-4 text-sm text-white/75">
                        Se o e-mail estiver cadastrado, você receberá um código temporário para criar uma nova senha.
                    </div>

                    <x-auth-session-status class="mb-4" :status="session('status')" />

                    <form method="POST" action="{{ route('password.email') }}"
                        x-data="{ seconds: {{ $resetCooldown }} }"
                        ...
                        <p x-show="seconds > 0" class="mb-4 text-xs text-cyan-100">
                            Você poderá pedir outro código em <span x-text="seconds"></span> segundos.
                        </p>
                        ...
                        <div class="flex justify-end">
                            <button type="submit"
                                ...
                                <span x-show="seconds <= 0">Enviar código</span>
