...
<head>
    ...
    <title>{{ config('app.name', 'Laravel') }} - Editar Roadmap</title>
    ...
    <header class="af-topbar">
        <div class="af-left">
            ...
            <div class="af-title-stack">
                ...
                <input id="roadmap-title" class="af-title-input" value="{{ old('titulo', $roadmap->titulo ?? $roadmap->title ?? $roadmap->nome ?? 'Roadmap sem titulo') }}" aria-label="Titulo do roadmap">
            </div>
        </div>
...
<script>
    ...
    window.roadmapMeta = {
        id: null,
        turmaId: "{{ $turma->id }}",
        storeUrl: "{{ route('roadmaps.update', [$turma->id ?? $turmaId ?? request()->route('turmaId'), $roadmap->id ?? $roadmapId ?? request()->route('roadmapId')]) }}",
...
<script id="af-edit-clone-runtime">
(() => {
    const editSaveUrl = @json(route('roadmaps.save', [
        $turma->id ?? $turmaId ?? request()->route('turmaId'),
        $roadmap->id ?? $roadmapId ?? request()->route('roadmapId')
    ]));

    const editShowUrl = @json(route('roadmaps.show', [
        $turma->id ?? $turmaId ?? request()->route('turmaId'),
        $roadmap->id ?? $roadmapId ?? request()->route('roadmapId')
    ]));

    const editCsrfToken = @json(csrf_token());

    const roadmapMeta = {
        title: @json($roadmap->title ?? $roadmap->titulo ?? $roadmap->nome ?? 'Roadmap sem titulo'),
        description: @json($roadmap->description ?? $roadmap->descricao ?? ''),
    };

    const storedRoadmap = {
        data: @json($roadmap->data ?? null),
        roadmap_data: @json($roadmap->roadmap_data ?? null),
        conteudo_json: @json($roadmap->conteudo_json ?? null),
        conteudo: @json($roadmap->conteudo ?? null),
        content: @json($roadmap->content ?? null),
        nodes: @json($roadmap->nodes ?? null),
        connections: @json($roadmap->connections ?? null),
    };
    ...
    const normalizeGraph = (value) => {
        ...
        if (Array.isArray(parsed.modules) && parsed.modules.length) {
            const modules = parsed.modules.map((module, index) => ({
                ...
                viewport: module.viewport || { x: 0, y: 0 },
            }));
            ...
            return {
                nodes: Array.isArray(parsed.nodes) ? parsed.nodes : active.nodes,
                connections: Array.isArray(parsed.connections) ? parsed.connections : active.connections,
                modules,
                activeModuleId,
                viewport: active.viewport || parsed.viewport || { x: 0, y: 0 },
            };
        }
        ...
        if (parsed.roadmap && (parsed.roadmap.nodes || parsed.roadmap.connections)) {
            return {
                nodes: Array.isArray(parsed.roadmap.nodes) ? parsed.roadmap.nodes : [],
                connections: Array.isArray(parsed.roadmap.connections) ? parsed.roadmap.connections : [],
            };
        }
    ...
    const getInitialGraph = () => {
        const candidates = [
            storedRoadmap.conteudo_json,
            storedRoadmap.roadmap_data,
            storedRoadmap.data,
            storedRoadmap.conteudo,
            storedRoadmap.content,
            {
                nodes: storedRoadmap.nodes,
                connections: storedRoadmap.connections,
            },
        ];

        const hasGraphContent = (graph) => {
            if (!graph) return false;

            return Boolean(
                graph.nodes?.length
                || graph.connections?.length
                || graph.modules?.some((module) => module.nodes?.length || module.connections?.length)
            );
        };
    ...
    const getFlow = () => window.ArtisanFlow
        || window.artisanFlow
        || window.artisanflow
        || window.artisanFlowEditor
        || window.ArtisanFlowEditor
        || null;

    const readTransformPosition = (node) => {
    ...
    const readDomGraph = () => {
        const nodes = [...document.querySelectorAll('#roadmap-canvas .roadmap-node')].map((node) => {
            const id = node.dataset.nodeId || node.dataset.id || node.id?.replace(/^node-/, '') || `node-${Date.now()}`;
            const title = node.querySelector('.af-node-title')?.textContent?.trim() || node.dataset.title || 'Elemento';
            const content = node.querySelector('.af-node-content')?.textContent?.trim() || node.dataset.content || '';
            const kind = node.querySelector('.af-node-kind')?.textContent?.trim() || node.dataset.kind || '';
            const image = node.querySelector('.af-node-image');
            const video = node.querySelector('.af-node-video');
            const download = node.querySelector('.af-download-link');
            const translated = readTransformPosition(node);

            return {
                id,
                type: [...node.classList].find((item) => item.startsWith('type-'))?.replace('type-', '') || node.dataset.type || 'task',
                kind,
                title,
                content,
                x: Number.parseFloat(node.style.left) || Number.parseFloat(node.dataset.x) || translated?.x || 0,
                y: Number.parseFloat(node.style.top) || Number.parseFloat(node.dataset.y) || translated?.y || 0,
                width: Number.parseFloat(node.style.width) || node.offsetWidth || 180,
                height: Number.parseFloat(node.style.height) || node.offsetHeight || 92,
                imageSrc: image?.getAttribute('src') || node.dataset.imageSrc || undefined,
                videoEmbedUrl: video?.getAttribute('src') || node.dataset.videoEmbedUrl || undefined,
                videoUrl: node.dataset.videoUrl || undefined,
                fileData: download?.getAttribute('href') || node.dataset.fileData || undefined,
                fileName: download?.getAttribute('download') || node.dataset.fileName || undefined,
            };
        });

        const connections = [...document.querySelectorAll('#af-connections [data-connection-id], #af-connections path')].map((path) => {
            const source = path.dataset.source
                || path.dataset.sourceId
                || path.dataset.from
                || path.getAttribute('data-source')
                || path.getAttribute('data-source-id')
                || path.getAttribute('data-from');

            const target = path.dataset.target
                || path.dataset.targetId
                || path.dataset.to
                || path.getAttribute('data-target')
                || path.getAttribute('data-target-id')
                || path.getAttribute('data-to');

            if (!source || !target) return null;

            return normalizeConnection({
                id: path.dataset.connectionId || path.id || `conn-${source}-${target}`,
                source,
                target,
            });
        }).filter(Boolean);
    ...
    const normalizeConnection = (connection) => {
        ...
        return {
            ...connection,
            id: connection.id || `conn-${source}-${target}`,
            source,
            target,
            sourceId: source,
            targetId: target,
    ...
    const mergeDomGraph = (graph) => {
        const domGraph = readDomGraph();
        const normalized = normalizeGraph(graph) || { nodes: [], connections: [] };
        const runtimeNodes = Array.isArray(normalized.nodes) ? normalized.nodes : [];
        const domNodes = Array.isArray(domGraph.nodes) ? domGraph.nodes : [];
        const runtimeById = new Map(runtimeNodes.map((node) => [node.id, node]));
        const nodes = domNodes.length
            ? domNodes.map((node) => ({ ...(runtimeById.get(node.id) || {}), ...node }))
            : runtimeNodes;
        const connections = (Array.isArray(normalized.connections) ? normalized.connections : [])
            .map(normalizeConnection)
            .filter(Boolean);

        return {
            nodes,
            connections: connections.length ? connections : domGraph.connections,
        };
    };
    ...
    const getCurrentGraph = () => {
        ...
        if (flow?.getSerializableGraph && typeof flow.getSerializableGraph === 'function') {
            return flow.getSerializableGraph();
        }

        if (flow?.getData && typeof flow.getData === 'function') {
            return mergeDomGraph(flow.getData());
        }

        if (flow?.data) {
            return mergeDomGraph(flow.data);
        }

        if (window.artisanFlowData) {
            return mergeDomGraph(window.artisanFlowData);
        }

        for (const key of ['roadmapData', 'roadmap_data', 'initialRoadmapData', 'initialData', '__artisanFlowData']) {
            if (window[key]) {
                return mergeDomGraph(window[key]);
            }
        }

        return mergeDomGraph(null);
    };

    window.getRoadmapEditPayload = getCurrentGraph;
    ...
    const renderFlow = () => {
        ...
        if (typeof flow.render === 'function') flow.render();
        if (typeof flow.renderNodes === 'function') flow.renderNodes();
        if (typeof flow.renderConnections === 'function') flow.renderConnections();
        if (typeof flow.updateSummary === 'function') flow.updateSummary();
        if (typeof flow.updateMinimap === 'function') flow.updateMinimap();
        if (typeof flow.syncForm === 'function') flow.syncForm();
    };

    const bootExistingRoadmap = () => {
        const graph = getInitialGraph();
        const flow = getFlow();

        if (!graph || !flow) return;

        if (typeof flow.loadGraph === 'function') {
            flow.loadGraph(graph);
            window.artisanFlowData = flow.getSerializableGraph?.() || graph;
            return;
        }
        ...
        window.artisanFlowData = flow.data;

        renderFlow();
    };

    const notify = (message, isError = false) => {
        const target = document.querySelector('[data-af-status], .af-save-status, .af-toast, .af-status');
    ...
    const saveRoadmap = async (options = {}) => {
        const shouldRedirect = options.redirect !== false;
        const graph = getCurrentGraph();

        const currentTitle = document.getElementById("roadmap-title")?.value?.trim()
            || roadmapMeta.title
            || 'Roadmap sem titulo';
        ...
        const payload = {
            titulo: currentTitle,
            title: currentTitle,
            descricao: roadmapMeta.description || '',
            description: roadmapMeta.description || '',
            data: graph,
        };

        sessionStorage.setItem('uniroad:lastEditSavePayload', JSON.stringify(payload));
        ...
        window.lastRoadmapSaveRequest = payload;
        window.lastRoadmapSaveResponse = responsePayload || responseText;

        if (!response.ok) {
            const detail = responsePayload?.message
                || JSON.stringify(responsePayload?.errors || responsePayload || responseText || {});

            throw new Error(detail || `HTTP ${response.status}`);
        }

        notify('Roadmap salvo.');
        const flow = getFlow();
        if (flow) {
            flow.captureSavedState?.();
            flow.allowNavigation = shouldRedirect;
        }

        if (shouldRedirect) {
            window.location.href = editShowUrl;
        }
    };

    window.saveRoadmapEdit = saveRoadmap;

    const bindEditSave = () => {
        let isSaving = false;

        const handleSaveEvent = async (event, source) => {
            ...
            try {
                ...
                console.error(error);
                alert('Não foi possível salvar o roadmap. Confira os campos e tente novamente.');
                notify('Não foi possível salvar o roadmap. Confira os campos e tente novamente.', true);
            } finally {
        ...
        const isSaveControl = (element) => {
            if (!element) return false;

            if (element.closest?.('#af-node-note-modal, .af-note-modal, #af-unsaved-modal, .af-unsaved-modal')) {
                return false;
            }
            ...
            const action = [
                ...
                element.dataset.afAction,
                element.dataset.saveRoadmap,
            ...
            const actionTokens = action.split(/[\s_-]+/).filter(Boolean);

            return label.includes('salvar')
                || value.includes('salvar')
                || action.includes('salvar')
                || actionTokens.includes('save')
                || element.matches('[data-af-save], [data-save-roadmap], button[type="submit"], input[type="submit"], .af-primary');
        };

        document.addEventListener('click', async (event) => {
            const button = event.target.closest('button, a, input, [role="button"]');

            if (!button) return;

            if (!isSaveControl(button)) return;

            await handleSaveEvent(event, button);
        }, true);

        document.addEventListener('submit', async (event) => {
            await handleSaveEvent(event, event.submitter || event.target.querySelector('[type="submit"]'));
        }, true);

        window.setTimeout(() => {
            document.querySelectorAll('button, a, input, [role="button"]').forEach((item) => {
                if (!isSaveControl(item)) return;

                const clean = item.cloneNode(true);

                clean.setAttribute('data-af-edit-save-bound', 'true');
                clean.removeAttribute('href');
                clean.setAttribute('type', 'button');
                clean.addEventListener('click', (event) => handleSaveEvent(event, clean), true);
    ...
    const boot = () => {
        window.setTimeout(bootExistingRoadmap, 80);
        window.setTimeout(bootExistingRoadmap, 250);
        window.setTimeout(bootExistingRoadmap, 800);
        window.setTimeout(bootExistingRoadmap, 1600);
        bindEditSave();
    };
